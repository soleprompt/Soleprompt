...
async function synthesizeOpenAiChunk(text: string): Promise<Buffer> {
...
export async function synthesizeSpeech(text: string): Promise<TtsResult> {
  ...
  for (const chunk of chunks) {
    if (provider === "openai") {
      buffers.push(await synthesizeOpenAiChunk(chunk));
    } else {
  ...
  const voiceId =
    provider === "openai"
      ? process.env.OPENAI_TTS_VOICE?.trim() || DEFAULT_OPENAI_VOICE
      : process.env.ELEVENLABS_VOICE_ID?.trim() || DEFAULT_ELEVENLABS_VOICE_ID;
